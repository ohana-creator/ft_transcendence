...
function RedefinirSenhaContent() {
  ...
  const rules = [
    { label: t.reset_password.password_rules.min_chars, ok: newPassword.length >= 8 },
    { label: t.reset_password.password_rules.uppercase, ok: /[A-Z]/.test(newPassword) },
    { label: t.reset_password.password_rules.lowercase, ok: /[a-z]/.test(newPassword) },
    { label: t.reset_password.password_rules.number, ok: /\d/.test(newPassword) },
  ]
...
function LoadingFallback() {
  return (
    <div className="min-h-screen bg-vaks-light-primary dark:bg-vaks-dark-primary flex items-center justify-center">
      <div className="w-8 h-8 border-4 border-vaks-light-purple-button/30 border-t-vaks-light-purple-button rounded-full animate-spin" />
    </div>
  )
}
