...
function DestaqueHero({ vaquinha }: { vaquinha: VaquinhaPublica }) {
  ...
  const percent = Math.min((vaquinha.arrecadado / vaquinha.meta) * 100, 100);
...
// ─── Mock Data ──────────────────────────────────────────────────────────
const mockVaquinhasPublicas: VaquinhaPublica[] = [
  {
    id: '/1',
    nome: 'Ajuda ao Abrigo Municipal',
    descricao: 'Campanha para renovar o abrigo municipal e oferecer melhores condicoes aos residentes que mais precisam.',
    meta: 5000,
    arrecadado: 3750,
    criador: 'Maria S.',
    categoria: 'Solidariedade',
    contribuidores: ['Ana', 'Pedro', 'Joao', 'Sofia', 'Miguel', 'Rita', 'Carlos', 'Beatriz'],
    imagem: 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=600&h=400&fit=crop',
    diasRestantes: 12,
    destaque: true,
  },
  {
    ...
    descricao: 'Vamos organizar o melhor festival de verao da cidade, com musica ao vivo, comida e muita diversao para todos!',
    meta: 8000,
    arrecadado: 4200,
    criador: 'Tiago R.',
    categoria: 'Eventos',
    contribuidores: ['Lucas', 'Marta', 'Andre', 'Diana', 'Hugo', 'Ines'],
    imagem: 'https://images.unsplash.com/photo-1533174072545-7a4b6ad7a6c3?w=600&h=400&fit=crop',
    diasRestantes: 25,
    destaque: false,
  },
  {
    ...
    descricao: 'Recolha de fundos para comprar material escolar para criancas de familias carenciadas da comunidade.',
    meta: 2000,
    arrecadado: 1890,
    criador: 'Prof. Lina',
    categoria: 'Educacao',
    contribuidores: ['Rui', 'Teresa', 'Filipe', 'Catarina', 'Diogo', 'Mariana', 'Paulo', 'Laura', 'Nuno'],
    imagem: 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=600&h=400&fit=crop',
    diasRestantes: 5,
    destaque: false,
  },
  {
    ...
    descricao: 'Financiar o torneio desportivo entre escolas, com premios e equipamentos para todas as equipas participantes.',
    meta: 3500,
    arrecadado: 1200,
    criador: 'Coach Bruno',
    categoria: 'Desporto',
    contribuidores: ['Marco', 'Vera', 'Simao', 'Clara'],
    imagem: 'https://images.unsplash.com/photo-1461896836934-bd45ba8fcaff?w=600&h=400&fit=crop',
    diasRestantes: 18,
    destaque: false,
  },
  {
    ...
    descricao: 'Projeto para transformar paredes cinzentas em obras de arte, dando cor e vida ao bairro inteiro.',
    meta: 4000,
    arrecadado: 2800,
    criador: 'Artista Zeze',
    categoria: 'Cultura',
    contribuidores: ['Tania', 'Ricardo', 'Adriana', 'Goncalo', 'Helena', 'Vasco', 'Leonor'],
    imagem: 'https://images.unsplash.com/photo-1499781350541-7783f6c6a0c8?w=600&h=400&fit=crop',
    diasRestantes: 30,
    destaque: false,
  },
  {
    ...
    descricao: 'Criar uma horta comunitaria onde vizinhos possam cultivar legumes frescos e partilhar conhecimentos.',
    meta: 1500,
    arrecadado: 1500,
    criador: 'Dona Rosa',
    categoria: 'Comunidade',
    contribuidores: ['Antonio', 'Fatima', 'Jose', 'Amelia', 'Manuel', 'Conceicao'],
    imagem: 'https://images.unsplash.com/photo-1416879595882-3373a0480b5b?w=600&h=400&fit=crop',
    diasRestantes: 0,
    destaque: false,
  },
];
...
export default function VaquinhasLayoutPage() {
  ...
  const categorias = useMemo(() => {
    const cats = Array.from(new Set(mockVaquinhasPublicas.map((v) => v.categoria)));
    return ['Todas', ...cats];
  }, []);

  const categoriaCounts = useMemo(() => {
    const counts: Record<string, number> = { Todas: mockVaquinhasPublicas.length };
    mockVaquinhasPublicas.forEach((v) => {
  ...
  const filteredVaquinhas = useMemo(() => {
    return mockVaquinhasPublicas.filter((v) => {
  ...
  const destaque = mockVaquinhasPublicas.find((v) => v.destaque);
    ...
    <div className="min-h-screen bg-vaks-light-primary dark:bg-vaks-dark-primary">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="space-y-8">
          ...
          {/* Stats */}
          <StatsBar vaquinhas={mockVaquinhasPublicas} />
          ...
          {restantes.length > 0 ? (
            viewMode === 'carousel' ? (
              <CardCarousel>
                {restantes.map((v, i) => (
                  <div key={v.id} className="flex-shrink-0 w-[340px]">
