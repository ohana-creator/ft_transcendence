...
export class LedgerController {
  ...
  transfer(@CurrentUser() user: any, @Body() dto: TransferDto) {
    return this.ledgerService.transfer(user.userId, dto);
  }
  ...
  registerWallet(
    ...
    return this.ledgerService.registerWallet(user.userId, walletAddress);
  }
  ...
  getHistory(@CurrentUser() user: any, @Query() query: LedgerQueryDto) {
    return this.ledgerService.getHistory(user.userId, query);
  }
