...
Publica ledger.mint.confirmed → Redis Stream
       ↓
Wallet Service reconcilia a transação local (on-chain metadata)
```

Se o mint on-chain falhar, o Ledger Service publica `ledger.mint.failed` e o Wallet Service aplica compensação automática na wallet local (`wallet.deposit.reversed`), mantendo sincronização entre saldo interno e blockchain.
