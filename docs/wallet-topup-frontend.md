...
## Resumo do Fluxo
1. Utilizador escolhe metodo de pagamento e valor.
...
5. Front acompanha estado do carregamento (polling curto + atualizacao da carteira).
6. Quando backend confirmar, saldo e historico sao atualizados.
...
Usar prefixo unico: `WalletTopupDebug`
...
## Exemplo de Implementacao (React/Next)

```ts
async function iniciarTopup({ amount, paymentMethod, mode, token }) {
  console.debug('WalletTopupDebug carregarCarteira:start', { amount, paymentMethod, mode });

  const payload = { amount, paymentMethod, mode };
  console.debug('WalletTopupDebug carregarCarteira:request', payload);
  ...
  console.debug('WalletTopupDebug carregarCarteira:raw-response', raw);

  if (!res.ok) {
    throw new Error(raw?.message ?? 'Falha ao iniciar topup');
  }
  ...
  console.debug('WalletTopupDebug carregarCarteira:normalized-response', normalized);

  // Nao manter loading infinito apos criar pedido externo
  if (normalized.checkoutUrl) {
    const popup = window.open(normalized.checkoutUrl, '_blank', 'noopener,noreferrer');
    if (!popup) {
      console.warn('WalletTopupDebug checkout:popup-blocked', { url: normalized.checkoutUrl });
